...
\begin{itemize}
	% Experimental, leaving as comment, in case.
...
Se la Thing espone le sue proprietà in modalità di scrittura e lettura, è possibile utilizzare un unico breve comando: altrimenti, è necessario procedere alla definizione dei singoli handler. Si che nel caso della \textbf{Thing \textit{esposta}}, essa contiene già tutte le variabili lette dalla Thing Description: infatti, per leggere e/o scrivere e/o osservare le diverse proprietà che espone una Thing è necessario avvalersi di tre metodi messi a disposizione dall'API:
\begin{lstlisting}[language=JavaScript]
	// Auto Read-Write in one line
	thing.writeProperty("propertyName", propertyValue)
...
È comunque comodo tenere un riferimento a variabili locali per delle verifiche interne, dovute ad un cambio in una variabile osservata, o per motivi di sviluppo e/o controllo. Il risultato di tutte queste considerazioni, includendo l'uso del modulo \texttt{common.js} porta ad una soluzione descritta nel codice \ref{lst:bulb.js}. Si ricorda che tutti i sorgenti sono disponibili in allegati.

\begin{lstlisting}[language=JavaScript,caption={Parti essenziali del codice sorgente di \texttt{bulb.js}},label=lst:bulb.js]]
...
\begin{itemize}
	...
	Imposta lo stato attuale della Thing. Ha necessità di specificare il \texttt{Content-Type} impostato a \texttt{application/json} e un \texttt{body} nel quale specificare il valore da impostare. Nel caso dell'impostazione dello stato, il \texttt{body} risulta essere come segue: 
	\begin{lstlisting}[language=JavaScript]
	  true
	\end{lstlisting}
...
Come definito precedentemente, anche qui si riconoscono i limiti del framework, stavolta dovuti alla scarsa documentazione relativa al protocollo REST che le Things create supportano. Anche in questo caso è stato già aperto un ticket di supporto\footnote{\url{https://github.com/eclipse/thingweb.node-wot/issues/373}} che cerca di mettere più in chiaro la necessità di avere ben note le API, soprattutto se la Thing creata espone effettivamente un'interfaccia RESTful.\\
...
\begin{itemize}
	...
	\item \textbf{Assenza di dettagliata documentazione}: il framework spesso fa riferimenti alla documentazione generica fornita come linee guida del W3C; generalmente questo non causa problemi con progetti piccoli, ma comincia a diventare complesso da gestire quando le variabili in campo sono molteplici:
		\begin{itemize}
			...
			\item non vi sono dettagli riguardanti la validazione dei dati che arrivano alla Thing: anche se impostato, di default il \texttt{form} viene completamente ignorato e non vi è alcun controllo sul \texttt{DataSchema} in arrivo. Vi è necessità di eseguire una selezione manuale di attributi validi e non.
