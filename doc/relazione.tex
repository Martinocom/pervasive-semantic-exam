...
\hyphenation{sil-la-ba-zio-ne thread findbugs package gradle}
...
\begin{itemize}
	\item introduzione ai concetti trattati in questa relazione (IoT, Smart Room, Smart Things, Conoscenza);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%non numera l'ultima pagina sinistra
\clearpage{\pagestyle{empty}\cleardoublepage}
\chapter{Web Semantico e Smart Things}           %crea il capitolo
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%non numera l'ultima pagina sinistra
\clearpage{\pagestyle{empty}\cleardoublepage}
\chapter{Creazione di Smart Things}           %crea il capitolo
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%imposta l'intestazione di pagina
\lhead[\fancyplain{}{\bfseries\thepage}]{\fancyplain{}{\bfseries\rightmark}}  

Considerando l'insieme delle possibilità scoperte, si procede di conseguenza alla definizione e creazione delle Things.

\section{Smart Descripted Thing}
È estremamente facile, al giorno di oggi, confondere un oggetto \quotes{\textit{connesso}} e un oggetto intelligente: in poche parole, mentre il primo è semplicemente accessibile tramite uno (o più) protocolli di comunicazione, offrendo a disposizione dell'utente i dati e funzioni che esso possiede, il secondo interagisce autonomamente con l'ambiente e altri oggetti eventualmente connessi, anche in modo autonomo. È utile, di conseguenza, coniare un nuovo termine, anche per il solo fine di questa relazione, e di definire una \textbf{Smart \textit{\ul{Descripted}} Thing} come l'entità che si vuole costruire su una piattaforma successivamente scelta.\\

Un primo accenno a cos'è una Smart Thing si può trovare nel capitolo \ref{sec:smart_thing}, ma una \textbf{Smart \textit{\ul{Descripted}} Thing} non è un semplice oggetto connesso: è un'entità capace di rendere la propria presenza partecipe al sistema, offrendo la possibilità di essere automaticamente scoperta e capita in tutti i suoi punti di vista; che si tratti di funzionalità, proprietà o eventi, essi devono essere accessibili dall'esterno tramite uno (o più) protocolli standard. L'oggetto così definito dev'essere descritto tramite una Thing Description, che include tutti gli aspetti sottolineati. Inoltre, la thing così creata, deve rispettare gli standard definiti dall'ente W3C per essere il più possibile compatibile e interoperabile coi sistemi attualmente esistenti.\\

Come ulteriore requisito, si pone quello di rendere le things capaci di offrire conoscenza su di esse, grazie all'inclusione di ontologie nella Thing Description. Gli utilizzatori finali possono utilizzare sia la conoscenza basilare, offerta dalla descrizione, sia la conoscenza aggiuntiva grazie alla quale è possibile comprendere di più sull'oggetto stesso (ed eventualmente, su altri oggetti presenti nell'ambiente).\\

Da questo momento in poi, \textbf{Smart \textit{\ul{Descripted}} Thing} e Smart Thing diventano sinonimi, se non diversamente specificato.
...
\subsection{Scelta delle Things}
Prima di passare alla progettazione, è opportuno selezionare alcuni oggetti della vita quotidiana che potrebbero essere in qualche modo rappresentativi per il modello che si vuole simulare. Trattandosi di un'esplorazione che ha come obiettivo quello di mettere il focus su una Smart Room, è di notevole importanza selezionare oggetti che possano risultare importanti per le analisi svolte.\\
