...
Come definito precedentemente, anche qui si riconoscono i limiti del framework, stavolta dovuti alla scarsa documentazione relativa al protocollo REST che le Things create supportano e all'assenza di un pieno supporto al controllo dei dati in arrivo: infatti, la documentazione non parla in alcun modo di protocollo REST, se non facendo riferimento allo standard W3C che generalizza il discorso. L'assenza di controllo dei dati di arrivo, invece, è un problema conosciuto e in fase di implementazione, in quanto non attualmente prioritario. Anche in questo caso è stato già aperto un ticket di supporto\footnote{\url{https://github.com/eclipse/thingweb.node-wot/issues/373}} che cerca di mettere più in chiaro la necessità di avere ben note le API, soprattutto se la Thing creata espone effettivamente un'interfaccia RESTful.\\
