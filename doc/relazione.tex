...
\hyphenation{sil-la-ba-zio-ne thread findbugs package gradle dev}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%non numera l'ultima pagina sinistra
\clearpage{\pagestyle{empty}\cleardoublepage}
\chapter{Analisi delle Things}           %crea il capitolo
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%imposta l'intestazione di pagina
\lhead[\fancyplain{}{\bfseries\thepage}]{\fancyplain{}{\bfseries\rightmark}}  
...
\begin{itemize}
	...
	\item \textbf{WoT-TD-Java} \cite{wot-td-java}: libreria in fase di sviluppo per la creazione della Thing Description usando RDF4J, Turtle e JSON-LD 1.0. Tenta di essere compliant allo standard suggerito dall'ente W3C riguardante la definizione della TD.
...
\texttt{WebThings.io}, dall'altro canto, con il suo \textbf{WebThings}, risulta essere una piattaforma (tecnicamente) completa sotto tutti i punti di vista, anche dei requisiti richiesti nel capitolo \ref{sec:iot_platform}. Si tratta infatti di un framework capace di gestire sia il punto d'ingresso delle things (il gateway) e la creazione delle thing stesse: a differenza del modulo della Samsung, Mozilla offre la possibilità di creare gli oggetti Smart attraverso l'uso di librerie, sia sviluppate dal team interno, sia dalla community \cite{webthings}. Lo sviluppatore ha quindi di fronte diverse scelte, sia nel campo dei linguaggi, nei quali le things possono essere implementate (Java, Python, Arduino per citare i più famosi, per passare anche da C\#, Go e PHP), sia per quanto riguarda la virtualizzazione delle stesse: non vi è infatti alcuna necessità di possedere board o schede dedicate per la prototipazione.\\

In ultima analisi, vi è una libreria creata pensando per un ambito più ampio, quello degli Agenti, ma che si adatta bene ad una classica programmazione grazie al fatto di essere una libreria Java per la creazione della Thing Description e, a differenza del framework di \texttt{WebThings.io}, risulta essere maggiormente compatibile con le indicazioni del W3C per quanto riguarda la definizione di quest'ultima.\\

Considerando validi entrambi gli ultimi due approcci, vi è necessità di trovare differenze che possano determinare in qualche modo l'effettiva superiorità di uno rispetto all'altro.

\begin{itemize}
	\item L'approccio considerato da \texttt{WebThings.io} risulta essere semplice e ben documentato, con un sito dedicato all'utilizzo sia della parte del Gateway che del Framework per le Things; dall'altra parte si può trovare un repository con una documentazione interna, non al livello di quella offerta da WebThings Framework.
	
	\item Vi sono diverse possibilità di scelta nel linguaggio e modo di implementare all'interno di \texttt{WebThings.io}, mentre la libreria offre il supporto soltanto a Java (ed eventualmente linguaggi che si basano su di esso, tipo Kotlin).
	
	\item \texttt{WebThings.io} utilizza una propria versione di API non pienamente compatibile con gli standard W3C, mentre la libreria cerca di essere quasi totalmente compliant alle raccomandazioni ufficiali.
\end{itemize}

Mentre entrambi i primi punti danno un'estrema forza al framework di \texttt{WebThings.io}, il terzo punto, analizzato in dettaglio, porta a dover completamente escludere la sua scelta. In particolare, vi è una netta incompatibilità con quello che è lo standard W3C e l'effettiva API della libreria. Si tratta infatti di un concetto estremamente importante e necessario ai fini del progetto, il quale può venire fuori soltanto leggendo la documentazione ufficiale di entrambe le specifiche.
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{eps/specification-conflict.png}
...
\subsection{Analisi del framework scelto}
...
\section{Definizioni delle Description}
Prima di procedere a codificare gli oggetti, è utile definire in principio le Thing Description caratterizzanti gli oggetti indicati nella sezione \ref{sec:modellazione-things}. Inoltre, come già definito nel capitolo \ref{sec:ontologie_note}, è possibile far riferimento a più documenti che definiscono lo standard: per essere compliant al framework, si preferisce utilizzare il riferimento di \texttt{webthings.io}, anche se in versione \quotes{\textit{non ufficiale}}. Inoltre, considerando lo \quotes{\textit{stato del documento}} relativo alla Web Thing API è \textbf{necessario} utilizzare quello come riferimento ufficiale, in quanto copre tutte le caratteristiche nella libreria sottostante: eventuali aggiunte messe a disposizione dal W3C (ovvero, le raccomandazioni ufficiali) potrebbero non essere supportate.\\

\quotes{\textit{This specification started as a draft W3C member submission by Mozilla, but continues to be maintained to document the API currently implemented by the WebThings IoT platform while it converges with the W3C standards as they mature.}}
...
Si parte da una descrizione senza considerare eventuali ontologie da creare; si predispone però quest'ultima già in modo da avere un contesto indicato, in modo da essere individuabile all'interno di uno schema definito di Things conosciute con proprietà simili.\\

Come contesti si possono utilizzare due piattaforme più famose: \href{https://webthings.io/schemas/}{\texttt{WebThings.io Schema}}\footnote{\url{https://webthings.io/schemas/}} oppure \href{http://iotschema.org/docs/full.html}{\texttt{IoT Schema}}\footnote{\url{http://iotschema.org/docs/full.html}}. Siccome la scelta del documento di riferimento è caduta anche precedentemente su \texttt{WebThings.io}, si preferisce continuare ad usare la stessa piattaforma anche per la definizione del contesto.\\

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{eps/light.png}
	\caption{Rappresentazione di funzionalità e attributi di una lampadina secondo \texttt{WebThings.io Schema}.}
	\label{fig:web-things-io-light-schema}
\end{figure}

Partendo come esempio dalla lampadina, essa è identificabile tramite lo schema come una \quotes{\textit{Light}}. Nella figura \ref{fig:web-things-io-light-schema} è possibile apprezzare le funzionalità e attributi richiesti.\\


Si definisce quindi la Web Thing come compliant al contesto di \texttt{WebThings.io Schema} e come tipologia oggetto come una \quotes{\textit{Light}}, utilizzando la formattazione JSON:
...
Segue la definizione di parametri necessari all'identificazione della Thing all'interno di un ambiente: vengono forniti un identificatore (in forma di URI), un titolo e una descrizione dell'oggetto. Si vuole notare come il campo \textit{id} della TD possa variare in base alla futura implementazione del gateway, nella parte in cui specifica l'indirizzo base (\texttt{https://mywebthingserver.com/}). \\

\begin{lstlisting}[language=json,caption={Attributi di base di una lampadina},label=lst:start2-td-lamp]
  [...]
...
Successivamente, per essere compliant allo schema indicato, si definisco le proprietà che una generica lampadina possiede. Si vuole notare che lo schema in figura \ref{fig:web-things-io-light-schema} dimostra \uline{tutte} le possibilità di un oggetto, ma soltanto alcune sono richieste nella fase di implementazione; altre sono semplicemente opzionali.\\

Ogni proprietà definita, se contiene un campo \texttt{@type} è compliant allo schema; di conseguenza deve indicare tutti i parametri richiesti, in modo da offrire l'universalità nella lettura della thing e dei suoi attributi. Da notare che esistono due campi con denominazione \textit{type}: il primo, con la chiocciola (@) serve per evidenziare il tipo di proprietà attribuita allo scheda scelto, la seconda definisce il tipo di dato che restituisce quella determinata proprietà.\\
...
\begin{lstlisting}[language=json,caption={Proprietà di una lampadina},label=lst:start3-td-lamp]
  "properties": {
	...
	"on": {
	  ...
	  "links": [{"href": "/things/bulb/properties/on"}]
	},
...
Si vuole sottolineare che è possibile specificare eventi scaturiti da una lampadina, ma nel caso studiato non vi è presente tale richiesta.
Infine, per completare la TD, vengono indicati tutti i link disponibili su quell'oggetto: in questo caso, vengono specificati collegamenti alle proprietà, azioni e una differente rappresentazione della lampadina, in formato HTML:\\

\begin{lstlisting}[language=json,caption={Link di una lampadina},label=lst:start5-td-lamp]
"links": [
  {
    "rel": "properties",
...
\subsection{Ventilatore}

\subsection{Tapparella}
